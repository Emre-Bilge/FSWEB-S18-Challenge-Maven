...
public class ValidateCard {
    ...
    public static void jokerTypeCardControl(List<Card> cards) {
        if (cards != null) {
            for (Card card : cards) {
                // Type null değilse JOKER kontrolü yapılır
                if (card.getType() != null && card.getType().equals(Type.JOKER)) {
                    if (card.getValue() != null || card.getColor() != null) {
                        // Value veya Color null değilse exception fırlatılır
                        throw new CardException("When type is JOKER, value and color must be null",HttpStatus.NOT_FOUND);
                    }
                }
            }
        }
